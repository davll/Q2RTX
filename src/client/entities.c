...
static void CL_AddPacketEntities(void)
{
    ...
    for (pnum = 0; pnum < cl.frame.numEntities; pnum++) {
		...
		if ((effects & EF_COLOR_SHELL) && vid_renderer->integer  == VID_REF_VKPT) {
        ...
        if ((effects & EF_COLOR_SHELL) && vid_renderer->integer == VID_REF_GL) {
			renderfx = adjust_shell_fx(renderfx);
...
static void CL_AddViewWeapon(void)
{
    ...
    if (shell_flags && vid_renderer->integer == VID_REF_GL) {
